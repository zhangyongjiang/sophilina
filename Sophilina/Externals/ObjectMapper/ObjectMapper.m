...
- (void)map:(NSDictionary*)source toObject:(id)target error:(NSError**)error {
  ...
  for (NSString *key in [source allKeys]) {
    ...
    if (property == NULL) {
        ...
        if ([source isKindOfClass:[NSDictionary class]] && [target isKindOfClass:[NSMutableDictionary class]]) {
            NSMutableDictionary* t = (NSMutableDictionary *)target;
            [t addEntriesFromDictionary:source];
            Class targetPropertyClazz = [self classForArrayWithKey:key target:target];
            NSLog(@"%@", targetPropertyClazz);
        }
        else {
        ...
        if(targetPropertyValue == nil) {
          *error = [self parsingErrorWithDescription:@"Could not create object from class: %@ for key: %@", targetPropertyClazz, key];
          continue;
            ...
            if ([targetPropertyValue isKindOfClass:[NSMutableDictionary class]]) {
                targetPropertyClazz = [self classForArrayWithKey:key target:target];
                if (targetPropertyClazz) {
                    NSMutableDictionary* dst = targetPropertyValue;
                    NSDictionary* src = value;
                    for (id dictKey in src) {
                        id srcValue = [src objectForKey:dictKey];
                        if ([srcValue isKindOfClass:[NSString class]] || [srcValue isKindOfClass:[NSNumber class]]) {
                            [dst setObject:srcValue forKey:dictKey];
                        }
                        else {
                            id object = [[[targetPropertyClazz alloc] init] autorelease];
                            [self map:srcValue toObject:object error:error];
                            [dst setObject:object forKey:dictKey];
                        }
                    }
                }
                else {
